...
Given(
  ...
  function () {
    // El producto se cargará cuando exista el repositorio en memoria.
  }
);
...
Given(
  ...
  function () {
    // El carrito se preparará cuando exista el servicio de carrito.
  }
);

When(
  'el usuario {int} modifica la cantidad del producto {int} a {int}',
  async function () {
    throw new Error('FASE RED: HU4 no tiene implementacion de produccion');
  }
);
